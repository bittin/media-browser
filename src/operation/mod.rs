...
fn handle_replace(
    ...
    let item_from = match super::parsers::item_from_path(file_from, IconSizes::default()) {
        Ok(ok) => ok,
        Err(err) => {
            log::warn!("{}", err);
    ...
    let item_to = match super::parsers::item_from_path(file_to, IconSizes::default()) {
        Ok(ok) => ok,
        Err(err) => {
            log::warn!("{}", err);
            return ReplaceResult::Cancel;
...
impl Operation {
    pub fn pending_text(&self, ratio: f32, state: ControllerState) -> String {
        ...
        match self {
            ...
            Self::Restore { items } => fl!(
                "restoring", items = items.len()
                //, progress = progress()
        ),
        }
    }
